...
const AgentMyProperties = () => {
  ...
  const { data: properties, isPending } = useQuery({
    queryKey: ["agent_properties", user?.email],
    queryFn: async () => {
            ...
            <div className="px-5 py-2 flex flex-col items-start gap-2 w-full">
              <div className="flex items-center w-full justify-between">
                ...
                <div className="flex items-center gap-2">
                  {
                    property?.property_status !== 'Rejected' && <Tooltip title="Update Property">
                    <button onClick={()=>navigate(`/dashboard/agent/update/${property?._id}`)} className="border border-primary rounded-full p-2 text-2xl text-primary font-bold">
              ...
              <img src={user?.photoURL} alt="" className="w-10 h-10 object-cover rounded-full border border-primary"/>
              <h1>{user?.displayName}</h1>
              </div>
              {
                property?.property_status === 'Pending' && <div className="flex items-center gap-1 text-yellow-500">
